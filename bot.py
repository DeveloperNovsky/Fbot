...
intents.guilds = True
intents.members = True  # Needed to resolve members from IDs
...
# Tracks the last batch added (list of user IDs)
last_batch = []
...
def extract_names_from_text(content: str):
    # When adding via raw text messages, use the text as "user ID"
    names = []
...
async def on_message(message):
    ...
    if message.content:
        ...
        if len(names) >= 2:
            # Track last batch using fake IDs (string names) for text-based entries
...
async def addt(ctx, member: discord.Member, tickets: int):
    ...
    save_entries()
    last_batch = [user_id] * tickets  # Track last batch
...
async def entries(ctx):
    """Display all entries with ticket counts."""
    if ctx.channel.id not in ALLOWED_CHANNELS:
        return
    ...
    for uid, count in sorted(raffle_entries.items()):
        ...
        summary.append(f"{display}: {count} ticket(s)")
