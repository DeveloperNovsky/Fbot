...
async def adddn(ctx, arg1: str, arg2: str = None):
    ...
    await ctx.send(
        ...
        f"User: **{username}**\n"
        f"Amount: `{value:,}` gp\n"
        f"User Total: `{donations_data['donations'][key]:,}` gp\n"
...
async def entries(ctx):
    if not raffle_entries:
        await ctx.send("🎟️ No raffle entries yet.")
        return
    ...
    for key, count in raffle_entries.items():
        ...
        lines.append(f"{name}: {count} ticket{'s' if count != 1 else ''}")
        total += count

    await ctx.send(
        f"🎟️ **Raffle Entries ({total} total tickets)**\n```" +
        "\n".join(lines) +
        "```"
    )
